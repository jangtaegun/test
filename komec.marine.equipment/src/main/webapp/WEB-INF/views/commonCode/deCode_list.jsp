...
<div class="top">
	...
	<ul class="search_input">
		<li><label>구분</label><input class="search_box01"  name="keyword2" id="keyword2" value="${keyword2}"    style="text" /></li>
		<li><label>구분 명</label><input class="search_box01" name="keyword" id="keyword" value="${keyword}" style="text" /></li>
	</ul>
	<div class="btn_wrap">
		<p class="btn01"><a href="" onClick="javascript:fn_submit(document.frm_list,'/deCodeList.do','post'); return false;">검색</a></p>
...
<div id="deCodeDetail" style="display: none;">
		<div class="top">
			<div class="btn_wrap">
				<p class="btn01">
					<input type="button" value="저장"  onclick="javascript:fn_submit(document.DecodeDetail,'/code_insert.do','post','deCode');" />
				</p>
	...
	function deCode_detailView(chnlId, useInitId, cmmnGbCd) {
		$.get("<c:url value='/DecodeDetailView.do'/>", { chnlId : chnlId, useInitId : useInitId, cmmnGbCd : cmmnGbCd }, function(res, code) {
		 ...
		 if(code=='success'){
			    ...
			    $("#cmmnGbCd").attr("readonly",true);
			    $("#cmmnGbCd").attr('title', '코드구분 코드는 수정할수 없습니다.');
			    $('#cmmnGbCd').poshytip({
